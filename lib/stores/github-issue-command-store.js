...
function targetReadToken(primaryToken) {
  const fallback =
    process.env.PILOT_TARGET_GITHUB_TOKEN ||
    process.env.GITHUB_TOKEN ||
    "";

  return fallback && fallback !== primaryToken ? fallback : "";
}

async function readPullPage(baseUrl, githubToken, githubRequest, page) {
  const url =
    `${baseUrl}/pulls?state=all&sort=created&direction=desc&per_page=${PAGE_SIZE}&page=${page}`;

  const primary = await githubRequest(url, githubToken);
  if (primary.ok || primary.status !== 404) return primary;

  const fallbackToken = targetReadToken(githubToken);
  if (!fallbackToken) return primary;

  return githubRequest(url, fallbackToken);
}

async function findPullByMarker(baseUrl, githubToken, githubRequest, marker) {
  for (let page = 1; page <= MAX_PAGES; page += 1) {
    const pulls = await readPullPage(
      baseUrl,
      githubToken,
      githubRequest,
      page,
    ...
    if (!pulls.ok) {
      if (pulls.status === 404) return null;
