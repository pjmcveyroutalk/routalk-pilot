...
function normalizeApproval(body) {
  const approval = body.approval;
  if (!approval || typeof approval !== "object" || Array.isArray(approval)) {
    fail("approval is required");
  }
  const basis = validateText(approval.basis, "approval.basis", 32, true);
  if (!["direct_request", "approved_proposal"].includes(basis)) {
    fail("approval.basis must be direct_request or approved_proposal");
  }
  const reference = validateText(approval.reference, "approval.reference", 240);
  if (basis === "approved_proposal" && !reference) {
    fail("approval.reference is required for approved_proposal");
  }
...
function normalizeCommand(body, options = {}) {
  ...
  const approval = normalizeApproval(body);
