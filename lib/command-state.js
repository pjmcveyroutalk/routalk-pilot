...
function createCommandRecord(command, details = {}) {
  ...
  if (typeof command.command_id !== "string" || !command.command_id) throw new Error("command_id is required");
...
function transitionCommandRecord(record, nextState, details = {}) {
  if (!record || typeof record !== "object" || !isCommandState(record.state)) throw new Error("record has an invalid state");
  if (!isCommandState(nextState)) throw new Error(`unknown command state: ${nextState}`);
  if (record.state === nextState) return record;
  if (TERMINAL_STATES.has(record.state)) throw new Error(`terminal command cannot transition from ${record.state}`);
  const allowed = TRANSITIONS[record.state];
  if (!allowed || !allowed.has(nextState)) throw new Error(`invalid command transition: ${record.state} -> ${nextState}`);
  return Object.freeze({
    ...record,
    state: nextState,
...
function workflowToCommandState(workflow, newPullRequestCount = 0) {
  ...
  if (workflow.status !== "completed") return COMMAND_STATES.RUNNING;
  if (workflow.conclusion !== "success") return COMMAND_STATES.FAILED;
  if (newPullRequestCount > 0) return COMMAND_STATES.AWAITING_APPROVAL;
  return COMMAND_STATES.COMPLETED;
}

function createCommandStoreAdapter(adapter) {
  if (!adapter || typeof adapter.findByCommandId !== "function" || typeof adapter.persist !== "function") {
  ...
  return Object.freeze({
    async enqueue(command, encryptedEnvelope) {
      ...
      let record = createCommandRecord(command, { storage: adapter.name || "unknown" });
